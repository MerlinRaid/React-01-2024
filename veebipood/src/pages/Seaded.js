...
function Seaded() {
  ...
  const sisestaEmail= () => {
    ...
    //vead:
    // 1)"npm start" ---> terminalis on viga (kompileerimis viga - koodi kokkupakkimine)
    //       leht on üleni  musta taustaga ja kirjutatakse meile mingi viga välja
    //2) parem klõps --> inspect -> console ---> console's lahti kirjutataud viga (run-time error - brauseris viga)
    //        leht on üleni valge, terminalis (npm start) on roheline
    //3) muutuja sees vale väärtus, valesti olen midagi kokku pannud
    //  probleemi leidmiseks ->  console.log("email fnkts käivitatud: " + emailRef.current.value);
